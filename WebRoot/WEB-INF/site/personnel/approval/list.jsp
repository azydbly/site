			...
			var a = "";
				<c:forEach items="${roleMenuId }" var="roleMenuId">
					<c:if test="${roleMenuId.operation.menuname eq 4 }">
						a += '<a title="编辑" href="javascript:;" onclick="edit_show_full([' + "'" + row.employee.userName + "'" + '] + \'的档案审批\',\'toCheckPage.action\',' + row.id + ')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
					</c:if>
				</c:forEach>
				if(a == ""){
