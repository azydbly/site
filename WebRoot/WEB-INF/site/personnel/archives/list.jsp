		...
		<span class="l">
			<c:forEach items="${roleMenuId }" var="roleMenuId">  
				<c:if test="${roleMenuId.operation.menuname eq 3 }">
					<a href="javascript:;" onclick="add_full('添加档案','toInsPage.action')" class="btn btn-primary radius">
						<i class="Hui-iconfont">&#xe600;</i> 添加档案
					</a>
				</c:if>
			</c:forEach>
		</span> 
...
$(function() {
	...
	datatable = $('.table-sort').DataTable({
		...
		columns: [{
			...
			defaultContent: "",
			render: function(data, type, row, meta) {
				return '<a class="view" onclick="edit_show_full([' + "'" + row.employee.userName + "'" + '] + \'的信息\',\'seeInfo.action\',' + row.id + ')">' + data + '</a>';
			}
		}, {
			data: "sex",
			render: function(data, type, row, meta) {
				...
				if(data == 4 || data == 5){
					<c:forEach items="${roleMenuId }" var="roleMenuId">
						<c:if test="${roleMenuId.operation.menuname eq 4 }">
							a += '<a title="编辑" href="javascript:;" onclick="edit_show_full([' + "'" + row.employee.userName + "'" + '] + \'的档案编辑\',\'toUpdate.action\',' + row.id + ')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
						</c:if>
					</c:forEach>
				}else if(data == 1){
					<c:forEach items="${roleMenuId }" var="roleMenuId">
						...
						<c:if test="${roleMenuId.operation.menuname eq 5 }">
							a += '<a title="删除" href="javascript:;" onclick="del([' + row.id + '],\'deleteOneById.action\',[' + "'" + row.employee.userName + "'" + '],reloadTable)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
						</c:if>
					</c:forEach>
				}else{
					a = "";
