...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont"></i> 首页 <span class="c-gray en">&gt;</span>招聘管理 <span class="c-gray en">&gt;</span>面试管理<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<div class="r">
			...
			<select id="resumeStatus" style="width:80px; height:30px;">
				...
				<option value="1" <c:if test="${resumeStatus == 1 }">selected</c:if>>推荐面试</option>
				<option value="2" <c:if test="${resumeStatus == 2 }">selected</c:if>>推荐二面</option>
				<option value="3" <c:if test="${resumeStatus == 3 }">selected</c:if>>推荐三面</option>
			</select>&nbsp;&nbsp;
...
$(function() {
	...
	datatable = $('.table-sort').DataTable({
		...
		columns: [{
			...
			defaultContent: "",
			render: function(data, type, row, meta) {
				return '<a class="view" onclick="edit_show_full([' + "'" + row.username + "'" + '] + \'的简历信息\',\'selectResumeManageById.action\',' + row.id + ')">' + data + '</a>';
			}
		}, {
			data: "sex",
			defaultContent: "",
			...
			render: function(data, type, row, meta) {
				...
				<c:forEach items="${roleMenuId }" var="roleMenuId">
					<c:if test="${roleMenuId.operation.menuname eq 4 }">
						a += '<a title="面试结果登记" href="javascript:;" onclick="change_result([' + "'" + row.username + "'" + '] + \'的面试结果登记\',\'forwardInterview.action\',' + row.id + ',' + data + ')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
					</c:if>
				</c:forEach>
				if(a == ""){
