...
<%@ include file="/WEB-INF/layouts/common/public.jsp" %> Transitional//EN">
<html>
<head>
<title>面试管理（二面）</title>
</head>
...
<article class="page-container">
	<form action="" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data" >
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box">
					<select class="select" size="1" name="resumeStatus">
						<option value="3"
							<c:if test="${resumeStatus == 3 }">selected</c:if>>推荐三面</option>
						<option value="4"
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" readonly name="recommendDateSecond" value="<fmt:formatDate value='${resumeManageDate }' pattern='yyyy-MM-dd'/>" />
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>二面面试评价：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea type="text" class="input-text textarea" name="recommendDescribeSecond">${resumeManage.recommendDescribeSecond }</textarea>
			</div>
		</div>
		<div class="row cl">
...
$(function(){
	...
	$("#form-member-add").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				type: 'post',
				url: "saveInterview.action?save=2",
				data: $(form).serialize(),
				dataType: "json",
