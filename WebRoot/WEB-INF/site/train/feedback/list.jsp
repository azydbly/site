...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont"></i> 首页 <span class="c-gray en">&gt;</span>培训管理 <span class="c-gray en">&gt;</span>培训计划反馈 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
$(function() {
	...
	datatable = $('.table-sort').DataTable({
		...
		ajax: {
			url: "showPageTrainPlan.action?save=3",
			type: 'post',
			data: function(d) {
				d.search = $('#search').val();
		...
		columns: [{
			...
			data: "tname",
			render: function(data, type, row, meta) {
				return '<a class="view" onclick="edit_show([' + "'" + data + "'" + '] + \'计划查看\',\'trainDetial.action\',' + row.id + ',\'500\',\'550\')">' + data + '</a>';
			}
		}, {
			data: "teacher",
			defaultContent: ""
				...
				if(data == 0){
					<c:forEach items="${roleMenuId }" var="roleMenuId">
						<c:if test="${roleMenuId.operation.menuname eq 4 }">
							a += '<a title="编辑" href="javascript:;" onclick="edit_show([' + "'" + row.tname + "'" + '] + \'计划反馈\',\'trainGofankui.action\',' + row.id + ',\'893\',\'860\')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
						</c:if>
					</c:forEach>
				}
				if(a == ""){
